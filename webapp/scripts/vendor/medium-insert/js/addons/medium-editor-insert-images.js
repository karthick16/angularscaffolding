...
(function ($) {
  ...
  $.fn.mediumInsert.registerAddon('images', {
    ...
    defaults: {
      ...
      */
      uploadFile: function ($placeholder, file, that) {
    	  var imageUploadPath;
    	  //If its hero editable,do something else.
    	  var heroEditable = $($placeholder).parents('.heroEditable');
    	  //console.log(heroEditable.length);
    	  if(heroEditable.length > 0 ){
    		  imageUploadPath = that.options.imagesUploadScript + "&heroTitle=true"; 
    	  }else{
    		  imageUploadPath = that.options.imagesUploadScript + "&heroTitle=false";
    	  }
        ...
        $.ajax({
          ...
          complete: function (jqxhr) {
            that.uploadCompleted(jqxhr, $placeholder,that);
          },
          processData: false,
    ...
    */
    ...
    uploadCompleted: function (jqxhr, $placeholder,that) {
      ...
      if (jqxhr.responseText) {
    	  console.log(that.options.heroImage);
    	  var heroEditable = $($placeholder).parents('.heroEditable');
    	  //console.log(heroEditable.length);
    	  if(heroEditable.length > 0 ){
    			//If this is for background image ,then do it as background image.
    	    	$('.hero-a-great-story').css("background",'url(' + jqxhr.responseText + ') no-repeat center center'); 
    	    	$('.hero-a-great-story').css("background-size","cover");
    	    	$('.hero-a-great-story').css("-webkit-background-size","cover");
    	  }else{
